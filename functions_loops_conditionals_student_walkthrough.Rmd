...
# Then we want a function converts from C to F:
c_to_f <- function(dat){
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    square_digits <- function(num){
      ...
      for (digit in unlist(strsplit(num_str, NULL))){
        if (grepl("^[0-9]$", digit)){
          squared_digit <- as.character(as.integer(digit)^2)
          squared_digits <- paste0(squared_digits, squared_digit)
        }
      }
      
      result <- as.integer(squared_digits)
      return(result)
    }
    ...
    binary_array_to_number <- function(arr){
      sum(arr*2^(rev(seq_along(arr)-1)))
    }

    binary_array <- c(1,0,0,1)
    print(binary_array_to_number(binary_array))
    ...
    add_length <- function(string){
      words <- strsplit(string, " ")[[1]]
      
      result <- vector("list", length = length(words))
      
      for (i in seq_along(words)){
        result[[i]] <- paste0(words[i], " ", nchar(words[i]))
      }
      return(result)
    }
    ...
    distances_from_average <- function(data){
      average <- mean(data)
      distances <- round(average - data, 2)
      return(distances)
    }

    data <- c(55,95,62, 36, 48)
    ...
    alphabet_postion <- function(text){
      positions <- c()
      text <- tolower(text)
      
      for (char in strsplit(text, NULL)[[1]]){
        if(grepl("[a-z]", char)){
          position <- as.integer(charToRaw(char)) - as.integer(charToRaw("a")) + 1
    ...
    print(alphabet_postion(input_text))
    ```
